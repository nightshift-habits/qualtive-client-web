...
export const present = (collection: string, options?: FormOptions): Form => {
  ...
  if (!_globalStyleElement || !_globalStyleElement.parentElement) {
    const styleElement = document.createElement("style")
    styleElement.innerHTML = style
    document.head.appendChild(styleElement)
    _globalStyleElement = styleElement
  }
      ...
      question.content.forEach((questionContent, contentIndex) => {
        switch (questionContent.type) {
          case "title": {
            ...
            titleElement.innerHTML = questionContent.text
            contentElement.appendChild(titleElement)
            break
          }
          case "score": {
